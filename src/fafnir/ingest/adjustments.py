...
def compute_for_security(db: Database, security_id: int) -> list[dict]:
    ...
    price_by_date: dict = defaultdict(lambda: 1.0)
    vol_by_date: dict = defaultdict(lambda: 1.0)

    for act in actions:
        ex_date = act["ex_date"]
        if act["action_type"] == "split":
            num = float(act["split_numerator"])
            den = float(act["split_denominator"])
            price_by_date[ex_date] *= den / num
            vol_by_date[ex_date] *= num / den
        elif act["action_type"] == "dividend":
            amount = float(act["dividend_amount"] or 0.0)
            if amount <= 0:
                continue
            ...
            if factor <= 0:
                repo.add_dq_flag(
                    ...
                    table_name="core.adjustment_factor",
                    record_key={"ex_date": str(ex_date)},
                    detail={"prior_close": p, "dividend": amount},
                )
                continue
